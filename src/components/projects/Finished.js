            ...
            <div className="col-12 col-sm-6 col-lg-3 mb-3" key={proj.id}>
              <div className="card card-body bg-light" id={proj.pClient.split(' ').join('-') + proj.id}>
                ...
                <div className="card-img-overlay">
                  <button type="button" className="btn btn-outline-warning btn-sm" data-toggle="modal" data-target={`.finished${proj.pClient.split(' ').join('-') + proj.id}`}>
                    View more ...
                  </button>
                </div>
              ...
              {/* MODAL */}
              <div className={`modal fade finished${proj.pClient.split(' ').join('-') + proj.id}`} tabIndex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel" aria-hidden="true">
