    ...
    return (
        <div className="flex flex-col gap-4 p-2 xl:p-4 overflow-y-auto h-[calc(60vh)] border-2 border-slate-300 rounded-b-xl my-2"
            ...
            {messages.map((message, key) => {
                const isCurrentUser = message.sender && message.sender._id === currUser;
                const messageClasses = `flex max-w-[70%] flex-col gap-2 rounded-l-xl rounded-tr-xl 
                    ${isCurrentUser ? "bg-blue-400 text-slate-100 ml-auto" : "bg-gray-300 text-slate-700 mr-auto"} 
                    p-2 xl:p-4 md:max-w-[75%] dark:bg-slate-800 dark:text-slate-300`;

                return (
                    <div key={key} className={`flex items-end gap-2 ${isCurrentUser ? "ml-auto" : "mr-auto"}`}>
                        ...
                        {message.sender ? (
                            <span className="flex size-9 items-center justify-center overflow-hidden rounded-full 
                                ...
                                {message.sender.picture?.url ? (
                                    <img src={message.sender.picture.url} alt="avatar" className="w-full h-full object-cover" />
                                ) : (
