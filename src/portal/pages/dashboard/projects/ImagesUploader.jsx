...
import { addProjectImages, getOneProject, deleteProjectImage } from '@/redux/slices/projectsSlice';
...
export function ImagesUploader() {
    ...
    const handleUpload = async () => {
        ...
        images.forEach(image => formData.append('pGallery', image));

        try {
            const response = await dispatch(addProjectImages({ formData, projectID }));
            if (response.payload) {
                setImages([]);
                setPreviewUrls([]);
            }
    ...
    const handleDeleteExistingImage = async (imageID) => {
        try {
            await dispatch(deleteProjectImage({ projectID, imageID }));
        } catch (error) {
        ...
        <Card className="p-4 w-full mx-auto">
            <CardBody>
                <Typography variant="h5" className="mb-4">
                    Upload Images to the "{project?.pName.split('')[0].toUpperCase() + project?.pName.slice(1)}" Project
                </Typography>
