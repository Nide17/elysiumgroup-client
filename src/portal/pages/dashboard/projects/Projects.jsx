        ...
        <CardBody className="overflow-x-scroll px-0 pt-0 pb-2">
          <table className="w-full min-w-[640px] table-auto">
            ...
            {isLoading ? (
              ...
              <tbody>
                {projects && projects.map(({ _id, pName, pClient, pStartDate, pType, status, pLocation, pGallery }, key) => {
                  const className = `py-3 px-2 ${key === projects.length - 1 ? "" : "border-b border-blue-gray-50"}`;
                  const firstGalleryImg = pGallery && pGallery.length > 0 ? pGallery[0].url : "/img/team-2.jpeg"

                  return (
                    <tr key={_id}>
                      <td className={className}>
                        <div className="flex items-center gap-4">
                          <Avatar src={firstGalleryImg} alt={pName ? pName : "Project Name"} size="sm" variant="rounded" />
                          <div>
